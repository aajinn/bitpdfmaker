...
export default function PDFViewer() {
        ...
        return (
                <main className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-cyan-100 font-sans flex flex-col">
                                ...
                                <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg border-t border-gray-200 z-50">
                                        <div className="max-w-4xl mx-auto px-2 sm:px-4">
                                                <div className="relative">
                                                        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-indigo-600 text-white px-3 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-medium shadow-md">
                                                                Pages
                                                        </div>
                                                        <div className="flex overflow-x-auto gap-1 sm:gap-2 p-3 sm:p-4 scrollbar-thin scrollbar-thumb-indigo-300 scrollbar-track-transparent">
                                                                {Array.from({ length: numPages }, (_, i) => i + 1).map((pageNum) => (
                                                                        <button
                                                                                key={pageNum}
                                                                                onClick={() => setCurrentPage(pageNum)}
                                                                                className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-lg border-2 transition-all ${currentPage === pageNum
                                                                                        ? 'bg-indigo-600 text-white border-indigo-600'
                                                                                        : 'bg-white text-indigo-600 border-indigo-200 hover:border-indigo-400'
                                                                                        }`}
                                                                        >
                                                                                <span className="text-xs sm:text-sm font-medium">{pageNum}</span>
                                                                        </button>
                                                                ))}
                                                        </div>
                                                </div>
                        ...
                        <footer className="text-bg-indigo-400 py-3 sm:py-6 text-center select-none text-xs sm:text-sm">
